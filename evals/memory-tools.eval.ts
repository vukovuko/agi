...
import { memoryTools } from "../src/agent/tools/index.ts";
import {
  toolsSelected,
  toolsAvoided,
  toolSelectionScore,
...
import dataset from "./data/memory-tools.json" with { type: "json" };
import { singleTurnExecutor } from "./executors.ts";

const executor = async (data: EvalData) => {
  return singleTurnExecutor(data, memoryTools);
};
...
evaluate({
  ...
  groupName: "memory-tools-selection",
});
